...
static float viewMargin = 5.0f; //按钮距离上边距
static float viewHeight = 40.0f; //按钮视图高度
...
- (id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(keyboardHide) name:@"keyboardHide" object:nil];
         ...
         [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(notification) name:@"DKSTextView" object:nil];
        //创建视图
        [self creatView];
    }
    return self;
...
- (void)changeFrame:(CGFloat)height {
    ...
    self.backView.frame = CGRectMake(0, 0, K_Width,self.frame.size.height);
...
- (void)keyboardWillShow:(NSNotification *)notification {
    ...
    [UIView animateWithDuration:duration delay:0 options:[notification.userInfo[UIKeyboardAnimationCurveUserInfoKey] integerValue] animations:^{
        self.frame = CGRectMake(0, endFrame.origin.y - self.backView.frame.size.height - StatusNav_Height, K_Width, self.frame.size.height);
        [self changeTableViewFrame];
    } completion:nil];
}

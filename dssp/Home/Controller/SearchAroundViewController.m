...
- (void)popToMapHomeNeedResetHome:(BOOL)need {
    for (NSInteger i = 0; i < self.navigationController.viewControllers.count; i++) {
        UIViewController *vc = self.navigationController.viewControllers[i];
        if ([vc isKindOfClass:NSClassFromString(@"MapHomeViewController")]) {
            if (need) {
                [(MapHomeViewController *)vc backToHome];
            }
...
- (void)btnClick:(UIButton *)sender {
    ...
    if (sender == _topBackBtn) {
        [self popToMapHomeNeedResetHome:NO];
    }
    if (sender == _clearBtn) {
        _searchField.text = @"";
        [self.annotations removeAllObjects];
    ...
    if (sender == _showClearBtn) {
        [self popToMapHomeNeedResetHome:YES];
    }
    ...
    if (sender == _infoFavoriteBtn) {
        if (sender.selected == NO) {
            weakifySelf
            [self addPoiWithName:self.currentPoi.name address:self.currentPoi.address location:self.currentPoi.coordinate tel:self.currentPoi.tel cpid:self.currentPoi.uid type:PoiTypeAll inResult:^(BOOL result, NSString *serviceId) {
                if (result) {
                    sender.selected = YES;
                    strongifySelf

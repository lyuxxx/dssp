...
/**
 设置y轴刻度的数值数组 传入的是[1,2,3,4] 表示是y轴刻度上表示的数值

 @param yLabels NSArray<NSString *>
 */
...
- (void)setYLabels:(NSArray *)yLabels withHeight:(CGFloat)height {
    ...
    if (_showLabel) {
        ...
        for (int index = 0; index < yLabels.count; index++) {
            ...
            NSInteger y = (NSInteger) (_chartCavanHeight + _chartMarginTop * 1.3 - index * yStepHeight);
...
/**
 设置x轴刻度的数值数组 传入的是[1,2,3,4] 表示是y轴刻度上表示的数值
 
 @param xLabels NSArray<NSString *>
 */
...
#pragma mark - Touch at point 触摸事件重写 目前没有用上
...
- (void)drawRect:(CGRect)rect {
    if (self.isShowCoordinateAxis) {
        ...
        // draw coordinate axis x/y轴的线的画法
...
#warning 这里把坐标轴尽头的箭头抹去了
//        // draw y axis arrow y轴尽头的箭头
...
//        CGContextStrokePath(ctx);
        ...
        if (self.showLabel) {
            #warning 这里 对x轴的刻度进行了偏移
            //_chartMarginBottom + _chartCavanHeight + 5
            // draw x axis separator 画x轴的刻度
            CGPoint point;
            ...
            // draw y axis separator  画y轴的刻度
//            CGFloat yStepHeight = _chartCavanHeight / _yLabelNum; // xLabel -> yLabel
...
- (void)setShadeBackground {
    ...
    [gradientLayer addSublayer:gradientLayerShade];
